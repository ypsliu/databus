...
import cn.rongcapital.mc2.data.bus.tools.mock.scenario.mocker.JourneyPresentMocker;
...
@Component
public class MockJourneyPresentProcess extends AbstractAsyncProcessor {

	@Value("${mock.journey.count:10}")
	private Integer journeyCount;

	@Autowired
	private JourneyPresentMocker journeyPresentMocker;

	@Override
	public void asyncProcess(Exchange exchange) {
		String presentBid = exchange.getProperty("SCENARIO_PRESENT_BID", String.class);
		...
		for (int i = 0; i < journeyCount; i++) {
			...
			journeyInfo.setEndBid(presentBid);
			journeyInfo.accept(journeyPresentMocker);
