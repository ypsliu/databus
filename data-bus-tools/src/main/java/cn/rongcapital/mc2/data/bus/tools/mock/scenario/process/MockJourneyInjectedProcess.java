...
import cn.rongcapital.mc2.data.bus.tools.mock.scenario.mocker.JourneyInjectedMocker;
...
@Component
public class MockJourneyInjectedProcess extends AbstractAsyncProcessor {

	@Value("${mock.journey.count:10}")
	private Integer journeyCount;

	@Autowired
	private JourneyInjectedMocker journeyInjectedMocker;
	...
	public void asyncProcess(Exchange exchange) {
		...
		for (int i = 0; i < journeyCount; i++) {
			...
			journeyInfo.accept(journeyInjectedMocker);
